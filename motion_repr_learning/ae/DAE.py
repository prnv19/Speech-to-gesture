...
class DAE:
    ...
    def __init__(self, shape, sess, variance_coef, data_info):
        ...
        with sess.graph.as_default():
            ...
            with tf.compat.v1.variable_scope("AE_Variables"):
                ...
                tf.compat.v1.add_to_collection('losses', self._reconstruction_loss)  # add weight decay loses
                self._loss = tf.add_n(tf.compat.v1.get_collection('losses'), name='total_loss')
    ...
    def _create_variables(self, i, wd):
        ...
        self[name_w] = tf.compat.v1.get_variable("Variables/"+name_w,
                                       initializer=tf.random.uniform(w_shape, -1 * a, a))
        ...
        tf.compat.v1.summary.histogram(name_w, self[name_w])

        # Initialize Train biases
        name_b = "bias"+str(i + 1)
        b_shape = (self.__shape[i + 1],)
        self[name_b] = tf.compat.v1.get_variable("Variables/"+name_b, initializer=tf.zeros(b_shape))

        if i < self.num_hidden_layers:
            # Hidden layer pretrained weights
            # which are used after pretraining before fine-tuning
            self[name_w + "_pretr"] = tf.compat.v1.get_variable(name="Var/" + name_w + "_pretr", initializer=
                                                      tf.random.uniform(w_shape, -1 * a, a),
                                                      trainable=False)
            # Hidden layer pretrained biases
            self[name_b + "_pretr"] = tf.compat.v1.get_variable("Var/"+name_b+"_pretr", trainable=False,
            ...
            self[name_b_out] = tf.compat.v1.get_variable(name="Var/"+name_b_out, initializer=b_init,
                                               trainable=True)
